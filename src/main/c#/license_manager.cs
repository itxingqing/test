    ...
    public class LicenseManager
    {
        ...
        public async Task<(bool success, string message)> ActivateAsync(string licenseKey)
        {
            try
            {
                ...
                HttpResponseMessage response = await _httpClient.PostAsync($"{SERVER_URL}/api/v2/activate", content);
        ...
        private async Task<(bool success, string message)> PerformHeartbeatAsync()
        {
            try
            {
                ...
                HttpResponseMessage response = await _httpClient.PostAsync($"{SERVER_URL}/api/v2/heartbeat", content);
                string responseBody = await response.Content.ReadAsStringAsync();
