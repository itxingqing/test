...
初始化是指首次部署服务器时，生成一组唯一的密钥对和盐值。这个步骤只需要执行**一次**，生成后的密钥将长期使用。

**为什么需要初始化？**

- RSA私钥用于服务器签名令牌，公钥用于客户端验证签名
- HKDF盐值用于增强密钥派生的安全性，防止彩虹表攻击
- 每个部署的服务器应该有唯一的密钥对，不要使用示例密钥
...
1. **RSA私钥**（Base64格式）- 配置到服务器的 `application.yml`
2. **RSA公钥**（Base64格式）- 需要转换后配置到客户端
3. **HKDF盐值**（Base64格式）- 配置到服务器的 `application.yml`

**重要提示：**
- ⚠️ 私钥和盐值**绝不能泄露**，只能存在于服务器端
- ⚠️ 不要将包含私钥的配置文件提交到Git
- ⚠️ 建议使用环境变量而不是直接写在配置文件中
- ⚠️ 将密钥保存到密码管理器或加密存储进行备份
